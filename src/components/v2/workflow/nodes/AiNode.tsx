...
type AiNodeProps = NodeProps<Node<WorkflowNodeData>>
...
export function AiNode({ data, selected }: AiNodeProps) {
  const execStatus = data.executionStatus || 'pending'
  const model = data.aiModel || 'sonnet'
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      >
        <div className="px-3 py-2.5">
          <div className="flex items-center gap-2 mb-1">
            <div className="w-6 h-6 rounded-md bg-gradient-to-br from-purple-500/20 to-blue-500/20 flex items-center justify-center flex-shrink-0">
              <Icon icon="lucide:sparkles" className="text-purple-400 text-xs" />
            </div>
          ...
          <div className="pl-8">
            {prompt ? (
              ...
              <p className="text-[10px] text-orange-400/80">Click to add prompt</p>
            )}
          </div>
      ...
      <Handle type="source" position={Position.Bottom} className="!w-2.5 !h-2.5 !bg-purple-500 !border-2 !border-pilos-bg" />
    </div>
  )
}
